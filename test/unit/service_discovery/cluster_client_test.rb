...
module ServiceDiscovery
  class ClusterClientTest < ActiveSupport::TestCase
    ...
    test 'discoverable services' do
      cluster.stubs(get_services: [
        cluster_service(metadata: { name: 'non-discoverable-api', uid: '220151f0-1918-4eca-808a-d4d07cea4b16', namespace: 'my-namespace' }, spec: { ports: [{ protocol: 'TCP', port: 3000, targetPort: 8080 }] }),
        cluster_service(
          metadata: {
          ...
          spec: { ports: [{ protocol: 'TCP', port: 3001, targetPort: 8080 }] }
        )
      ])
    ...
    test 'project with discoverables' do
      ...
      cluster.expects(:get_services).with(namespace: 'project-1').returns([
        cluster_service(metadata: { name: 'non-discoverable-api-1', uid: '220151f0-1918-4eca-808a-d4d07cea4b16', namespace: 'project-1' } )
      ])

      cluster.expects(:get_services).with(namespace: 'project-2').returns([
